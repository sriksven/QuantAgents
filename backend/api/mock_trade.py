...
async def get_mock_portfolio(user_id: str = "default", db: AsyncSession = Depends(get_db)):
...
@router.post("/fund")
async def add_mock_funds(request: FundRequest, user_id: str = "default", db: AsyncSession = Depends(get_db)):
    if request.amount <= 0:
...
@router.post("/order")
async def execute_mock_order(order: OrderRequest, user_id: str = "default", db: AsyncSession = Depends(get_db)):
